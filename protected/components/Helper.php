...
class Helper
 {
  ...
  public static function displayColor($color)
  {
   ...
   public function  getStatus($value,$yes,$no)
   {
    return $value==1 ? $yes : $no ;

   }
   public static function  categoryMainImg($id)
   {
    return '/media/types/'.strtolower(Category::model()->findByPk($id)->title).'/'.Category::model()->findByPk($id)->image;

   }
   public static function  categoryDesc($id)
   {
    return strip_tags(Category::model()->findByPk($id)->desc);
   }
