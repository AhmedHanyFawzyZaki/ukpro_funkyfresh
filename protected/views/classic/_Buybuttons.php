...
<form name="tmp_product" id="tmp_product" method="post">
    ...
    <input type="hidden" name="_id" id="_id" value="<?php if($tmp_product->id) echo $tmp_product->id; ?>">
    ...
    <input type="hidden" name="tmp_knit_stripe_left"  id="tmp_knit_stripe_left" value="<?php if($tmp_product->knit_stripe_left) echo $tmp_product->knit_stripe_left; ?>">  
     ...
     <!-- knit double strip-->
    <input type="hidden" name="tmp_double_stripe_color" id="tmp_double_stripe_color" value="<?php // if($tmp_product->double_stripe) echo $tmp_product->double_stripe; ?>">
    ...
    <input type="hidden" name="tmp_back_text_color2" id="tmp_back_text_color2" value="<?php if($tmp_product->back_text_color2) echo $tmp_product->back_text_color2; ?>">
     <input type="hidden" name="tmp_back_text_type2" id="tmp_back_text_type2" value="<?php if($tmp_product->back_text_type2) echo $tmp_product->back_text_type2; ?>">
      ...
      <input type="hidden" name="tmp_back_type" id="tmp_back_type" value="<?php if($tmp_product->back_type) echo $tmp_product->back_type; ?>">
    
      
      <input type="hidden" name="tmp_top_zip" id="tmp_top_zip" value="<?php if($tmp_product->top_zip) echo $tmp_product->top_zip; ?>">
    <input type="hidden" name="tmp_tail_color" id="tmp_tail_color" value="<?php if($tmp_product->top_zip) echo $tmp_product->top_zip; ?>">
    <input type="hidden" name="main_image" id="main_image" value="<?php if($tmp_product->main_image) echo $tmp_product->main_image; ?>" />
</form> 
...
//}
?>
  <?php if(!isset($tmp_product) or $tmp_product==null or Yii::app()->user->id == ''){ ?>
...
<!--<a href="<?php // echo Yii::app()->getBaseUrl(true)."/home/checkout/id/$tmp_product->id" ?>"   class="btn buy" ><i>buy now</i></a>-->
     <a href="<?php echo Yii::app()->getBaseUrl(true)."/home/cart"  ?>"  class="btn buy" ><i>buy now</i></a>
     <?php } ?>
...
function save_div(flag)
{
  ...
  onrendered: function(canvas) {
	...
	$.ajax({
                ...
                type: 'POST',
                data: { file : canvas.toDataURL("image/png"), "id":<?php  echo $id ?>},
                success: function(data, status){
		
			   console.log(data);
                           $("#main_image").val(data);
                         //   window.location.href="<?php echo Yii::app()->getBaseUrl(true)."/home/checkout/id/$id" ?>";
            }
            });
...
//    console.log(form_data);
        ...
        $.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    window.location.href="<?php echo Yii::app()->getBaseUrl(true)."/classic/index/id/" ?>"+data['id']+"/cat_id/"+data['cat_id']+"/subcat_id/"+data['subcat_id'];
